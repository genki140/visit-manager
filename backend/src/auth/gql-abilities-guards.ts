  ...
  canActivate(context: ExecutionContext): Promise<boolean> {
    return new Promise((resolve, reject) => {
      ...
      if (!requiredAbilities) {
        if (this.reflector.get<boolean | undefined>('noRequiredAbility', context.getHandler()) === true) {
          resolve(true);
        } else {
          resolve(false);
        }
...
// });
// カレントユーザーは以下のような方法で取得可能にしたい
// async users(@CurrentUser() user: User) {

export const NoRequiredAbility = () => SetMetadata('noRequiredAbility', true);
